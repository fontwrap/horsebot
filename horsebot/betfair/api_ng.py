__version__ = 0.07
__AUTHOR__ = 'birchy@bespokebots.com'
__LICENCE__ = 'http://creativecommons.org/licenses/by-nc/4.0/'
...
### NOTE ###
# Betfair uses UTC/GMT times for ALL datetime objects (market start times, etc)
# You should use Python's datetime.utcnow() for all dates & times
# It is recommended that you synchronise your PC clock using an NTP server
...
class API(object):
    ...
    def __init__(self, aus = False, ssl_prefix = '', locale = ''):
        ...
        """
        self.requests = requests.Session() # persist http connections
        self.convert_price = convert_price # ref to price_mod function
        self.get_price_spread = get_price_spread # ref to price_mod function
    ...
    def load_ssl_cert_paths(self, ssl_prefix = ''):
        ...
        if not ssl_prefix:
            ...
            msg += '*** HINT: this may be your username ***'
            raise Exception(msg)
        cert_paths = []
    ...
    def send_http_request(self, url = '', data = ''):
        ...
        # send request
        if data: # POST
            resp = self.requests.post(url, data, cert = ssl_cert, headers = headers, timeout = 60)
        else: # GET
            resp = self.requests.get(url, cert = ssl_cert, headers = headers, timeout = 60)
        # check response
    ...
    def get_markets(self, params = None, req_id = 1):
        ...
        if type(resp) is dict and 'result' in resp:
            ...
            for market in markets:
                try:
                    obj_date = datetime.strptime(market['marketStartTime'], '%Y-%m-%dT%H:%M:%S.%fZ')
                    market['marketStartTime'] = obj_date
                except:
                    pass
            return markets
        else:
    ...
    def get_market_books(self, market_ids = None, price_data = None,
        ...
        if type(resp) is dict and 'result' in resp:
            ...
            for book in books:
                try:
                    obj_date = datetime.strptime(book['lastMatchTime'], '%Y-%m-%dT%H:%M:%S.%fZ')
                    book['lastMatchTime'] = obj_date
                except:
                    pass
            return books
        else:
...
# listMarketProfitAndLoss
    ...
    def place_bets(self, market_id = '', bets = None, customer_ref = '', req_id = 1):
        ...
        if type(resp) is dict and 'result' in resp:
            resp = resp['result']
            if 'instructionReports' in resp:
                for bet in resp['instructionReports']:
                    try:
                        obj_date = datetime.strptime(bet['placedDate'], '%Y-%m-%dT%H:%M:%S.%fZ')
                        bet['placedDate'] = obj_date
                    except:
                        pass
            return resp
        else:
    ...
    def get_current_bets(self, market_ids = None, bet_ids = None, projection = 'ALL', req_id = 1):
        """simplified version of listCurrentOrders.
        returns json containing dict of current bets for given markets.
        current bets dict: keys = market ids, values = list of bets
        @market_ids: type = list. OPTIONAL list of market ids (MAX = 250 ids).
        @bet_ids: type = list. OPTIONAL list of bet ids (MAX = 250 ids).
        NOTE: if combining market_ids and bet_ids, MAX = 250 ids.
        @projection: type = string. must be one of the following:
            'ALL': return ALL bets (matched and unmatched)
            'EXECUTABLE': return only unmatched bets
            'EXECUTION_COMPLETE': return only FULLY matched bets
        @req_id: type = integer. OPTIONAL request id number (for customer use only)
        """
        url = 'https://api.betfair.com/exchange/betting/json-rpc/v1'
        if self.aus: url = 'https://api-au.betfair.com/exchange/betting/json-rpc/v1'
        if not market_ids: market_ids = []
        if not bet_ids: bet_ids = []
        req = {
            'jsonrpc': '2.0',
        ...
        if market_ids: req['params']['marketIds'] = market_ids
        if bet_ids: req['params']['betIds'] = bet_ids
        req = json.dumps(req) # convert dict to json format
        resp = self.send_http_request(url, req)
        if type(resp) is dict and 'result' in resp:
            # parse bets & group by selection ids
            bets = {}
            if 'currentOrders' in resp['result']:
                for order in resp['result']['currentOrders']:
                    try:
                        obj_date = datetime.strptime(order['placedDate'], '%Y-%m-%dT%H:%M:%S.%fZ')
                        order['placedDate'] = obj_date
                    except:
                        pass
                    market_id = order['marketId']
                    if market_id not in bets: bets[market_id] = {}
                    sel_id = order['selectionId']
                    if sel_id not in bets[market_id]: bets[market_id][sel_id] = []
                    bets[market_id][sel_id].append(order)
            return bets
        else:
            raise Exception(str(resp))

    def cancel_bets(self, market_id = '', bet_ids = None, req_id = 1):
        """cancel bets
        NOTES:
        * if market_id is supplied and bet_ids are empty, ALL bets will be
          cancelled on that market
        * if BOTH market_id AND bet_ids are empty, ALL exchange bets will be cancelled
        @market_id: type = string
        @bet_ids: type = list. values = bet ids
        @req_id: type = integer. OPTIONAL request id number (for customer use only)
        """
        url = 'https://api.betfair.com/exchange/betting/json-rpc/v1'
        if self.aus: url = 'https://api-au.betfair.com/exchange/betting/json-rpc/v1'
        ...
        if market_id:
            ...
            if bet_ids:
                ...
                for bet_id in bet_ids:
                    temp = {'betId': bet_id}
                    params['instructions'].append(temp)
        # build request
        req = {
            'jsonrpc': '2.0',
