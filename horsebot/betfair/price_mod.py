...
def convert_price(old_price = 0.0, round_up = True, pips = 0):
    ...
    """
    old_price = round(old_price, 2) # avoids 1.7400002 getting rounded to 1.75, etc.
    ...
    if old_price in price_increments:
        ...
        # find nearest price >= betfair price (i.e. default to round UP)
    ...
    if (not round_up # round DOWN
        and old_price not in price_increments # old price between betfair prices
        ):
        index -= 1
    index += pips
    if index < 0:
